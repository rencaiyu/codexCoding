  ...
  return (
    <>
      <Space style={{ marginBottom: 16 }}>
        {hasPermission("permissions:create") && <Button type="primary" onClick={() => { setEditing(null); form.setFieldsValue(defaultForm); setOpen(true); }}>新增权限</Button>}
      </Space>
      <Table rowKey="id" columns={[
        ...
        { title: "名称", dataIndex: "name" },
        {
          ...
          render: (value) => <Tag color={value === "MENU" ? "blue" : "green"}>{value === "MENU" ? "菜单" : "按钮"}</Tag>
        },
      ...
      <Modal title={editing ? "编辑权限" : "新增权限"} open={open} onCancel={() => setOpen(false)} onOk={submit} destroyOnClose>
        <Form form={form} layout="vertical" initialValues={defaultForm}>
          ...
          <Form.Item label="权限类型" name="permissionType" rules={[{ required: true }]}><Select options={permissionTypeOptions} /></Form.Item>
